    ...
    public class GameManager {
        ...
        // Játékos ID a multiplayerben
        private int myPlayerId = -1;

        // Egyéb játékosok
        private Map<Integer, Player> otherPlayers = new HashMap<>();

        // Új: Setter a path debug beállításhoz
        ...
        private void initGameplay(String playerName, Player.Ability ability) {
            ...
            // Átadjuk a path debug beállítást az ellenségeknek
            for (Enemy enemy : currentDungeon.getEnemies()) {
                enemy.setTargetPlayer(this.player);
                enemy.setShowPathDebug(this.showPathDebug); // Új sor
            }
...
//            );
        ...
        // Új metódus a mentett játék indításához
        public void startGameFromSave(int saveId) {
            this.saveIdToLoad = saveId;
            this.currentState = GameState.LOAD_GAME;
        }

        // Segédmetódus a saveId lekéréséhez
        private int getSelectedSaveIdFromSomewhere() {
            // Itt visszaadjuk a Lobby-ból kapott saveId-t
            // Ehhez tárolnunk kell egy változót
            return this.saveIdToLoad;
        }

        // Változó a betöltendő saveId tárolásához
        ...
        private void initGameplayFromSave(int saveId) {
            try {
                ...
                // DEBUG: Felülírás után milyen értékek vannak
        ...
        private void handleUpgradeChoice() {
            if (upgradeScreen.isChoiceMade()) {
                ...
                switch (choice) {
                    case 0:
                        System.out.println("KURVA ANYÁD PLAYER ALAP HPJA: " + player.getHealth());
                        player.addHealthBoost(20f);
                        System.out.println("KURVA ANYÁD PLAYER HPJA: " + player.getHealth());
                        System.out.println("KURVA ANYÁD PLAYER MAX HPJA: " + player.getMaxHealth());
                        break;
                    case 1:
        ...
        private void handleGameOver() {
            ...
            if (gameOverScreen.isSaveGame()) {
                // A GameOverScreen már elvégzi a tényleges mentést a saveGame() metódussal
                // Itt csak reseteljük a flag-et
        ...
        private void update(float deltaTime, double currentTime) {
            if (currentState == GameState.ABILITY_SELECTION) {
                ...
                if (abilitySelectionScreen.isSelectionComplete()) {
                    initGameplay(abilitySelectionScreen.getPlayerName(), abilitySelectionScreen.getSelectedAbility());
                }
                ...
                // Betöltjük a mentett játékot
                if (saveIdToLoad != -1) {

                    // 1. Inicializálja a Player-t, a Dungeon-t, és beállítja a Player-t az alapértelmezett helyre.
                    initGameplayFromSave(saveIdToLoad);

                    // 2. ✅ ÚJ HÍVÁS: Betölti a Player, Enemy, Boss valós statisztikáit az inicializált objektumokra.
                    loadSavedGame(saveIdToLoad);

                    // Reseteljük az ID-t, hogy ne töltődjön be újra
                    saveIdToLoad = -1;

                    // 3. Átváltunk a játékmenetre
                    currentState = GameState.GAMEPLAY;
                } else {
                    // Ha valamiért nincs ID, visszatérünk a menübe.
                    currentState = GameState.LOBBY;
                }
            }
        }
        ...
        private void render() {
            ...
            if (currentState == GameState.ABILITY_SELECTION) {
                ...
                // HUD renderelése - külön transzformációval
        ...
        private void cleanupForNextLevel() {
            ...
            // Textúrák törlése (újratöltjük őket)
            if (playerIdleTexture != null) {
                playerIdleTexture.delete();
                playerIdleTexture = null;
            }
            ...
            // Dungeon törlése (újat generálunk)
            if (currentDungeon != null) {
                currentDungeon.cleanup();
                currentDungeon = null;
            }

            // Dinamikus listák ürítése
            if (projectiles != null) {
                projectiles.clear();
            }
        ...
        private void cleanup() {
            try {
                ...
                // CSAK HA NEM NULL, akkor hívjuk meg a free()-t
        ...
        private void loop() {
            ...
            while (!glfwWindowShouldClose(window)) {
                ...
                // VÁLTOZTATÁS: Egér lenyomva tartására is lőjön
        ...
        public void startMultiplayerGame(String playerName, String ability, String serverIp, String serverPort, boolean isHost) {
            System.out.println("🎮 GameManager: Multiplayer játék indítása...");
            System.out.println("Player: " + playerName + " | Ability: " + ability);
            System.out.println("Server: " + serverIp + ":" + serverPort + " | Host: " + isHost);

            // Először inicializáljuk a játékot (mint a single player-ben)
            Player.Ability playerAbility = Player.Ability.valueOf(ability);
            initGameplay(playerName, playerAbility);

            // Majd inicializáljuk a multiplayer kapcsolatot
        ...
        // Multiplayer update a game loopban
        private void updateMultiplayer(float deltaTime) {
            if (!isMultiplayer || multiplayerClient == null) return;

            // Küldjük az inputot a szervernek
            if (currentState == GameState.GAMEPLAY) {
                sendPlayerInput();
            }

            // Fogadjuk a szerver üzeneteit
            processServerMessages();
        }

        private void sendPlayerInput() {
            // Gyűjtsük össze a játékos inputját
            String inputData = player.getX() + "," + player.getY() + "," +
                    inputHandler.getMovementX() + "," + inputHandler.getMovementY();
        ...
        private void updateFromGameState(String gameStateData) {
            ...
            try {
                // Feldolgozzuk a szervertől kapott játékállapotot
            ...
            for (String entry : playerEntries) {
                if (!entry.isEmpty()) {
                    ...
                    if (playerState != null && playerState.getPlayerId() != myPlayerId) {
                        // Frissítsd az egyéb játékosok pozícióját
        ...
        private void updateOtherPlayer(PlayerState playerState) {
            if (!otherPlayers.containsKey(playerState.getPlayerId())) {
                ...
                // ... egyéb tulajdonságok frissítése
            }
        }

        private Player createOtherPlayer(PlayerState playerState) {
            // Hozz létre egy új Player objektumot a szerver adatai alapján
            // Ez csak egy példa - a tényleges implementáció a játék struktúrájától függ
        ...
        private void updateEnemiesFromServer(String enemiesData) {
            // Frissítsd az ellenségeket a szerver adatai alapján
            // Ez a játék logikájától függ
        }

        private void updateProjectilesFromServer(String projectilesData) {
            // Frissítsd a lövedékeket a szerver adatai alapján
        }

        private void handlePlayerJoined(String data) {
            // playerId:name:ability formátum
            ...
            if (parts.length >= 3) {
                ...
                // Ha ez egy új játékos (nem mi), akkor hozz létre neki egy Player objektumot
