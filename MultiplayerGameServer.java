...
import world.ServerDungeonGenerator;
...
public class MultiplayerGameServer {
    ...
    private void generateSharedDungeon() {
        ...
        // Használd a ServerDungeonGenerator-t, ami csak 2 paramétert vár
        Dungeon dungeon = ServerDungeonGenerator.generateRandomDungeon(53, rand);

        gameState.setDungeon(dungeon);
        gameState.setDungeonGenerated(true);
        gameState.setBossDefeated(false);

        float baseSpawnX = dungeon.getPlayerSpawnX();
        float baseSpawnY = dungeon.getPlayerSpawnY();

        for (PlayerSession session : connectedPlayers.values()) {
            float offsetX = (rand.nextFloat() - 0.5f) * 50.0f;
            float offsetY = (rand.nextFloat() - 0.5f) * 50.0f;
        ...
        List<Enemy> dungeonEnemies = dungeon.getEnemies();
        int enemyId = 1;
        for (Enemy enemy : dungeonEnemies) {
            String type = (enemy instanceof Boss) ? "boss" : "normal";
            float health = (enemy instanceof Boss) ? 500.0f : 100.0f;
            EnemyState enemyState = new EnemyState(enemyId++, type, enemy.getX(), enemy.getY(), health, health);
            gameState.getEnemyStates().add(enemyState);
            System.out.println("👹 Added enemy: ID=" + enemyState.getEnemyId() + ", Type=" + type);
        }

        System.out.println("✅ Shared dungeon generated with " + connectedPlayers.size() + " players and " + gameState.getEnemyStates().size() + " enemies");
        broadcastTCPMessage("DUNGEON_SEED:" + seed);
    }

    private void broadcastDungeonData() {
        // CSAK EGY METÓDUS LEGYEN!
        Dungeon dungeon = gameState.getDungeon();
        if (dungeon != null) {
            StringBuilder dungeonData = new StringBuilder("DUNGEON_DATA:");
            dungeonData.append("player_spawn:").append(dungeon.getPlayerSpawnX())
                    .append(",").append(dungeon.getPlayerSpawnY());
            // További dungeon adatok...
            broadcastTCPMessage(dungeonData.toString());
        }
    }
